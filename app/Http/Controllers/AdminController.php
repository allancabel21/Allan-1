...
class AdminController extends Controller
{
    ...
    {
        $pendingJobs = JobPosting::where('status', 'pending')
            ->with('postedBy')
            ->orderBy('created_at', 'desc')
            ->paginate(10);

        $approvedJobs = JobPosting::where('status', 'approved')
            ->with(['postedBy', 'reviewedBy'])
            ->orderBy('reviewed_at', 'desc')
            ->paginate(10);

        $rejectedJobs = JobPosting::where('status', 'rejected')
            ->with(['postedBy', 'reviewedBy'])
            ->orderBy('reviewed_at', 'desc')
            ->paginate(10);

        return view('admin.job-review', compact('pendingJobs', 'approvedJobs', 'rejectedJobs'));
    }
    ...
    public function approveJob(Request $request, $id)
    {
        ...
        return redirect()->route('admin.job-review')->with('success', 'Job approved successfully!');
    }
    ...
    public function rejectJob(Request $request, $id)
    {
        ...
        return redirect()->route('admin.job-review')->with('success', 'Job rejected successfully!');
    }
